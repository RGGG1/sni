...
const USE_HOBO_LIST = retrieveEnvVariable('USE_HOBO_LIST', logger) === 'true';
const HOBO_LIST_REFRESH_INTERVAL = Number(retrieveEnvVariable('HOBO_LIST_REFRESH_INTERVAL', logger));
...
function shouldBuy(key: string): boolean {
  return USE_HOBO_LIST ? snipeList.includes(key) : true;
}
...
const runListener = async () => {
  ...
  if (USE_HOBO_LIST) {
    setInterval(loadSnipeList, HOBO_LIST_REFRESH_INTERVAL);
  }
};
